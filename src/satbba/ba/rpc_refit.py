...
def refit_rpc(
    ...
    image_id = str(getattr(image, "image_id", "unknown"))
    return RPCRefitResult(image_id=image_id, rmse_pixel=rmse, coeffs=coeffs)


def _safe_id(image_id: str) -> str:
    safe = re.sub(r"[^A-Za-z0-9._-]+", "_", image_id.strip())
    return safe or "unknown"


def export_refined_rpc(result: RPCRefitResult, output_dir: Path) -> Path:
    """Write refined RPC sidecar file with stable unique image id."""

    output_dir.mkdir(parents=True, exist_ok=True)
    safe_id = _safe_id(result.image_id)
    path = output_dir / f"image_{safe_id}.rpc"
    payload = {
